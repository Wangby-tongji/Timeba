...
config = dict()
"""Train"""
config["display_iters"] = 57977
config["val_iters"] = 57977 * 2
config["save_freq"] = 12.0
config["epoch"] = 0
...
config["train_split"] = os.path.join(
    root_path, "highD/highD3_5/train"
)
config["val_split"] = os.path.join(root_path, "highD/highD3_5/val")
config["test_split"] = os.path.join(root_path, "highD/highD3_5/test")
...
class ActorNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        for i in range(len(num_blocks)):
            ...
            n_in = n_out[i]
        self.groups = nn.ModuleList(groups) #self.groups有3个group，每个group有1个实例块，最后一个group维度是[M, 64, 5]
        ...
        for i in range(len(n_out)):
            self.Unet.append(Unet1d(n_out[i], 64))
    ...
    def forward(self, actors: Tensor) -> Tensor:
        ...
        out = self.lateral[-1](outputs[-1]) #M, 64, 5
