...
config = dict()
"""Train"""
config["display_iters"] = 44554
config["val_iters"] = 44554 * 2
...
config["train_split"] = os.path.join(
    root_path, "exiD/exiD3_5/train"
)
config["val_split"] = os.path.join(root_path, "exiD/exiD3_5/val")
config["test_split"] = os.path.join(root_path, "exiD/exiD3_5/test")
...
class AttDest(nn.Module):
    ...
    def forward(self, agts: Tensor, agt_ctrs: Tensor, dest_ctrs: Tensor) -> Tensor:
        ...
        dist = (agt_ctrs.unsqueeze(1) - dest_ctrs).view(-1, 2) #[B, 1, 2]-[B, 6, 2]
        dist = self.dist(dist)
