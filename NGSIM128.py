...
class ActorNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        for i in range(len(num_blocks)):
            ...
            n_in = n_out[i]
        self.groups = nn.ModuleList(groups) #self.groups有3个group，每个group有1个实例块，最后一个group维度是[M, 128, 5]
        ...
        for i in range(len(n_out)):
            self.Unet.append(Unet1d(n_out[i], 128))
    ...
    def forward(self, actors: Tensor) -> Tensor:
        ...
        out = self.lateral[-1](outputs[-1]) #M, 128, 5
