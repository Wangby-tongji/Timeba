...
class ArgoverseForecastingLoader:
    ...
    def track_id_list(self) -> List[int]:
        ...
        """
        _track_id_list: List[int] = np.unique(self.seq_df["id"].values).tolist()
    ...
    def agent_traj(self) -> np.ndarray:
        ...
        """
        agent_x = self.seq_df[self.seq_df["type"] == "AGENT"]["x"]
        agent_y = self.seq_df[self.seq_df["type"] == "AGENT"]["y"]
